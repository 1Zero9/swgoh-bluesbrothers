...
**Doc version:** 1.16.1 · **Last updated:** 2026-08-24 · tracks site `v0.24.0`
...
- **Drag-and-drop defence assignment.** Defence mode's "Available defenders" chip list and each assignment row are native HTML5 drag sources; zone tabs and the selected zone's assignment list/empty-state are drop targets. Dropping a player chip on a zone calls the existing `onCreateAssignment` with the player's first eligible squad (`bestSquadForPlayer`); dropping an existing assignment row on a different zone tab calls `onUpdateAssignment` with the new `zoneId`. An inline per-row squad `<select>` lets officers correct the auto-picked squad after the fact. `lib/tw-plans.ts`'s `updateAssignment()` only persists a `zoneId` change when `planId` is also present in the same call, so `tw-workspace.tsx` always passes `planId` through its `onUpdateAssignment` wrapper. The original manual "Player"/"Squad" dropdown form is kept as a fallback, since native HTML5 drag-and-drop doesn't work on touch devices.
